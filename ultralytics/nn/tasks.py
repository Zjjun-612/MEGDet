...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        Supports dual-stream fusion models (RGB/IR): when ChannelSelect layers are detected,
        pretrained backbone weights are automatically remapped to both RGB and IR streams.
        ...
        # --- Dual-stream fusion pretrained weight remapping ---
        # Detect ChannelSelect layers indicating a dual-stream (RGB/IR) backbone
        channel_select_indices = []
        for i, m in enumerate(self.model):
            if isinstance(m, ChannelSelect):
                channel_select_indices.append(i)

        if len(channel_select_indices) >= 2:
            # Find the first layer fed by each ChannelSelect (start of each stream)
            rgb_cs_idx = channel_select_indices[0]
            ir_cs_idx = channel_select_indices[1]
            rgb_start, ir_start = None, None
            for i, m in enumerate(self.model):
                f = getattr(m, "f", None)
                if isinstance(f, int):
                    if f == rgb_cs_idx and rgb_start is None:
                        rgb_start = i
                    elif f == ir_cs_idx and ir_start is None:
                        ir_start = i
                if rgb_start is not None and ir_start is not None:
                    break

            if rgb_start is not None and ir_start is not None:
                backbone_len = ir_start - rgb_start  # number of backbone layers per stream
                LOGGER.info(
                    f"Dual-stream fusion model detected: "
                    f"RGB backbone layers [{rgb_start}-{rgb_start + backbone_len - 1}], "
                    f"IR backbone layers [{ir_start}-{ir_start + backbone_len - 1}], "
                    f"mapping pretrained layers [0-{backbone_len - 1}] to both streams"
                )
                # Remap pretrained backbone weights to both RGB and IR streams
                remapped_csd = {}
                for k, v in csd.items():
                    remapped_csd[k] = v  # keep original key
                    if k.startswith("model."):
                        ...
                        try:
                            ...
                            continue
                        if 0 <= layer_idx < backbone_len:
                            old_prefix = f"model.{layer_idx}."
                            if k.startswith(old_prefix):
                                suffix = k[len(old_prefix):]
                                rgb_key = f"model.{layer_idx + rgb_start}.{suffix}"
                                ir_key = f"model.{layer_idx + ir_start}.{suffix}"
                                remapped_csd[rgb_key] = v
                                remapped_csd[ir_key] = v
                csd = remapped_csd
        # --- End of dual-stream fusion remapping ---
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        yaml_ch = self.yaml.get("ch") or self.yaml.get("channels")
        if yaml_ch and yaml_ch > 0 and ch == 3 and yaml_ch != ch:
            ch = yaml_ch
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, YOLOESegment, Pose, OBB, FourierHead)) else self.forward(x)
    ...
    def init_criterion(self):
        ...
        if isinstance(self.model[-1], FourierHead):
            return v8FourierLoss(self)
...
class FourierModel(DetectionModel):
    """YOLO Fourier detection model for arbitrary shapes."""

    def __init__(self, cfg="yolo11n-fce.yaml", ch=3, nc=None, verbose=True):
        """Initialize YOLO Fourier model with given config and parameters."""
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)

    def init_criterion(self):
        """Initialize the loss criterion for the FourierModel."""
        return v8FourierLoss(self)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m is FAAFusionConcat:
            c_high = ch[f[0]]
            c_low = ch[f[1]]
            dim = args[0] if len(args) > 0 else 1
            win = args[1] if len(args) > 1 else 7
            c_mid = args[2] if len(args) > 2 else 16
            gamma = args[3] if len(args) > 3 else 1e-3

            args = [c_high, c_low, dim, win, c_mid, 1e-8, gamma]
            c2 = c_high + c_low
        elif m is SFF:
            # Shallow Feature Fusion: need channel info
            c1 = ch[f[0]]  # Get channels from first input
            c2 = c1  # Output channels same as input
            args = [c1, *args]  # [channels, reduction]
        elif m in frozenset({MSDeformFusionBlock, WaveletGuidedMSDeformFusionBlock}):
            # MSDeformFusionBlock / WaveletGuidedMSDeformFusionBlock: deformable attention-based deep fusion
            c1 = ch[f[0]]
            c2 = c1
            args = [c1, *args]  # [channels, d_model, num_heads, d_ffn, dropout, n_points]
        elif m is ChannelSplit:
            # ChannelSplit splits input channels, output channels depend on split indices
            c1 = ch[f]
            split_idx = args[0] if args else c1 // 2
            # For simplicity, assume output is the first split's channels
            c2 = split_idx if isinstance(split_idx, int) else split_idx[0]
        elif m is ChannelSelect:
            # ChannelSelect: args are [start_ch, end_ch]
            c2 = args[1] - args[0]  # Output channels = end_ch - start_ch
        elif m is AddFusion:
            # AddFusion: 逐元素相加融合，需要输入通道列表和输出通道数
            c1_list = [ch[x] for x in f]  # 获取所有输入的通道数
            c2 = args[0]  # 输出通道数从配置中获取
            args = [c1_list, c2]  # [输入通道列表, 输出通道数]
        elif m is ConcatFusion:
            # ConcatFusion: 通道拼接融合，需要输入通道列表和输出通道数
            c1_list = [ch[x] for x in f]  # 获取所有输入的通道数
            c2 = args[0]  # 输出通道数从配置中获取
            args = [c1_list, c2]  # [输入通道列表, 输出通道数]
        elif m is SEFusion:
            # SEFusion: SE注意力融合，需要输入通道列表、输出通道数和reduction
            c1_list = [ch[x] for x in f]
            c2 = args[0]
            args = [c1_list, c2, *args[1:]]  # [输入通道列表, 输出通道数, reduction(可选)]
        elif m is CBAMFusion:
            # CBAMFusion: CBAM注意力融合，需要输入通道列表、输出通道数、reduction和kernel_size
            c1_list = [ch[x] for x in f]
            c2 = args[0]
            args = [c1_list, c2, *args[1:]]  # [输入通道列表, 输出通道数, reduction(可选), kernel_size(可选)]
        elif m is CrossAttentionFusion:
            # CrossAttentionFusion: 交叉注意力融合，需要两个输入通道和输出通道数
            c1_list = [ch[x] for x in f]
            c2 = args[0]
            args = [c1_list, c2, *args[1:]]  # [输入通道列表, 输出通道数, num_heads(可选)]
        elif m is DeformConvFusion:
            # DeformConvFusion: 可变形卷积融合，需要输入通道列表、输出通道数、k(可选)、use_mask(可选)
            c1_list = [ch[x] for x in f]
            c2 = args[0]
            args = [c1_list, c2, *args[1:]]  # [输入通道列表, 输出通道数, k(可选), use_mask(可选)]
        elif m in frozenset(
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, FourierHead}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, FourierHead}:
                m.legacy = legacy
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if "fourier" in m:
            args = cfg["head"][-1][-1]
            ne = 0
            if isinstance(args, list) and len(args) > 1 and isinstance(args[1], int):
                ne = args[1] if len(args) > 2 else int(args[1] == 1)
            return "obb" if ne > 0 else "detect"
        if m == "obb":
            return "obb"
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "pose"
            elif isinstance(m, OBB) or (type(m).__name__ == "FourierHead" and getattr(m, "ne", 0) > 0):
                return "obb"
