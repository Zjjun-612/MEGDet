...
from ultralytics.utils.fourier import inverse_fourier_transform
...
class FourierHead(Detect):
    """YOLO Fourier head for arbitrary shape detection using Fourier Contour Embedding.

    This class extends the Detect head to include Fourier coefficient prediction for contour representation.
    It also supports oriented bounding boxes (OBB) if `ne` is provided.

    Attributes:
        K (int): Order of Fourier series.
        nk (int): Total number of Fourier parameters (2*(2K+1)).
        ne (int): Number of extra box parameters (e.g., 1 for OBB angle).
        cv4 (nn.ModuleList): Convolution layers for Fourier coefficients.
        cv5 (nn.ModuleList): Convolution layers for angle prediction (if ne > 0).
    """

    def __init__(self, nc: int = 80, ne: int = 0, K: int = 3, ch: tuple = ()):
        """Initialize FourierHead with number of classes `nc`, extra parameters `ne`, Fourier order `K`, and layer channels `ch`.

        Args:
            nc (int): Number of classes.
            ne (int): Number of extra parameters (e.g. 1 for OBB angle) or Fourier order K if only one extra arg is
                provided in the model YAML.
            K (int): Order of Fourier series.
            ch (tuple): Tuple of channel sizes from backbone feature maps.
        """
        # parse_model appends ch to YAML args, so support:
        #   FourierHead, [nc]        -> (nc, ch)
        #   FourierHead, [nc, K]     -> (nc, K, ch) for K > 1
        #   FourierHead, [nc, ne]    -> (nc, ne, ch) for ne == 1
        #   FourierHead, [nc, ne, K] -> (nc, ne, K, ch)
        ...
        if not isinstance(ch, (list, tuple)) or not ch:
            raise ValueError("FourierHead requires a non-empty channel list `ch` from parse_model.")
        if not isinstance(ne, int) or ne < 0:
            raise ValueError(f"ne must be a non-negative integer, but got {ne}.")
        if not isinstance(K, int) or K < 1:
            raise ValueError(f"K must be a positive integer Fourier order, but got {K}.")
        
        super().__init__(nc, ch)
        self.nc = nc
        self.ne = ne
        self.K = K
        self.nk = 2 * (2 * K + 1)  # number of Fourier coefficients
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor | tuple:
        """Concatenate and return predicted bounding boxes, class probabilities, and Fourier coefficients."""
        bs = x[0].shape[0]  # batch size
        f_coeffs = torch.cat([self.cv4[i](x[i]).view(bs, self.nk, -1) for i in range(self.nl)], 2)
        ...
        res = Detect.forward(self, x)

        if self.training:
            return (res, angle, f_coeffs) if angle is not None else (res, f_coeffs)

        # Cartesian Fourier coefficients are learned in feature-map units during training.
        # Scale them by stride when exposing them with image-space detections.
        f_coeffs_scaled = f_coeffs * self.strides
        self.f_coeffs = f_coeffs_scaled

        if self.export:
            # Keep angle last because rotated NMS/predictor treat the final extra channel as theta.
            return torch.cat([res, f_coeffs_scaled, angle], 1) if angle is not None else res

        y, x_raw = res
        if angle is not None:
            # OBB mode: expose Fourier coefficients to NMS/validator while keeping angle as the last extra channel.
            return (torch.cat([y, f_coeffs_scaled, angle], 1), (x_raw, angle, f_coeffs))
        
        # HBB mode: keep the standard YOLO output for NMS/validation; Fourier remains a training auxiliary.
        return (y, (x_raw, f_coeffs))

    def decode_bboxes(self, bboxes: torch.Tensor, anchors: torch.Tensor) -> torch.Tensor:
        """Decode bounding boxes (either standard or rotated)."""
        if self.ne > 0:
            return dist2rbox(bboxes, self.angle, anchors, dim=1)
        return super().decode_bboxes(bboxes, anchors)

    def decode_fourier(self, f_coeffs: torch.Tensor, num_points: int = 100, absolute: bool = False) -> torch.Tensor:
        """Decode Fourier coefficients to Cartesian contour samples."""
        if absolute:
            raise RuntimeError(
                "Fourier coefficients decode to contours relative to their matched anchor/center. "
                "Absolute contour decoding requires adding the anchor or box center outside decode_fourier()."
            )
        # f_coeffs: (bs, nk, anchors)
        bs, _, anchors = f_coeffs.shape
        f_coeffs = f_coeffs.permute(0, 2, 1).view(bs * anchors, 2 * self.K + 1, 2)
        points = inverse_fourier_transform(f_coeffs, num_points=num_points, K=self.K)
        points = points.view(bs, anchors, num_points, 2)
        return points
