...
class RotatedTaskAlignedAssigner(TaskAlignedAssigner):
    ...
    def __init__(
        ...
        beta: float = 6.0,
        eps: float = 1e-9,
        iou_mode: str = "probiou",
    ):
        """Initialize rotated assigner with selectable localization metric."""
        super().__init__(topk=topk, num_classes=num_classes, alpha=alpha, beta=beta, eps=eps)
        self.iou_mode = str(iou_mode).lower()

    def iou_calculation(self, gt_bboxes, pd_bboxes):
        """Calculate IoU for rotated bounding boxes."""
        if self.iou_mode in {"angle", "angle_reg", "angle_param", "param"}:
            return bbox_iou(
                xywh2xyxy(gt_bboxes[..., :4]),
                xywh2xyxy(pd_bboxes[..., :4]),
                xywh=False,
                CIoU=True,
            ).squeeze(-1).clamp_(0)
